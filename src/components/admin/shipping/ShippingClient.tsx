...
import { Shipping, UserPermission } from '@/lib/types';
...
export default function ShippingClient({ shippings, permissions }: Props) {
  const {
    settings: { currency_symbol: currencySymbol },
  } = useSettingsStore();
  const [methods, setMethods] = useState<Shipping[]>(shippings);
