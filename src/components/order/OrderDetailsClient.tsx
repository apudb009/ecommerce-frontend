...
import { Order, OrderStatus, ReturnStatus, Tracking } from '@/lib/types';
...
export default function OrderDetailsClient({ order: initOrder, tracking }: Props) {
  ...
  const [isReturnRequested, setIsReturnRequested] = useState(!!initOrder.returnRequest?.status);
  ...
  const handleDownloadInvoice = async () => {
    ...
    try {
      ...
      try {
        ...
        const invoice = existing.find((inv: any) => inv.orderId === order.id);
        if (invoice) {
          invoiceId = invoice.id;
        } else {
          const { data: created } = await api.post(`/invoices/order/${order.id}`);
  ...
  return (
    <div className="max-w-3xl">
      ...
      <Actions
        ...
        disabledReturnRequestButton={isReturnRequested}
      ...
      {showReturnModal && (
        <ReturnModal
          ...
          onSubmitted={(returnStatus: ReturnStatus) => {
            setShowReturnModal(false);
            setIsReturnRequested(!!returnStatus);
          }}
        />
      )}
    </div>
